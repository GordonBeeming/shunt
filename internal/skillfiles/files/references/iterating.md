...
To refresh the SDK baked into existing guests, first upgrade/select a supported host SDK, run `{{shunt-command}} init --force` to rebuild the shared base image, then run plain `{{shunt-command}} reapply <siding>` followed by `{{shunt-command}} up <siding>`. Plain `reapply` preserves the worktree, branch, and siding data. Never use `--fresh-data` for an SDK-only refresh because that option intentionally replaces the siding's data from the baseline.
...
**Resolving a name is not the same as reaching it.** A private address behind a VPN resolves in the guest and then times out, because macOS does not forward the container bridge into the tunnel. `hostReach` in the contract is what covers that: the host does the connecting on the guest's behalf, and the app keeps using the real name and the real port. Reaching for a hosts entry instead makes the name resolve and the connection still hang, which is the worse failure of the two because it looks fixed. See [commands](commands.md) for the declaration.

## Cleaning up sidings without losing work

Run `{{shunt-command}} cleanup` from the registered repo when several sidings are finished. It lists every siding for that project, lets you select more than one, and names why protected work needs confirmation: dirty/untracked files, a recorded or checked-out branch owned by a surviving siding, or committed work not proven to survive locally (including Git's preservation reason). Preservation accepts exact reachability, strict per-commit equivalence after rebase/cherry-pick, and a whole topic range matching one squash commit on local `origin/HEAD` (falling back to `origin/main`). It rejects partial or multi-squash inference, merge ambiguity, detached/missing branches, and scan overflow. Recorded and checked-out branches are proved independently, and a branch owned by a surviving siding cannot be retired. After selection, a protected siding gets a second confirmation before Shunt removes its guest, worktree, journaled exact branch refs, and data.
...
Text and JSON retain the literal `uniqueCommits` value and add conservative nested preservation evidence aggregated across recorded and checked-out refs.

Each JSON preservation target has a stable `kind` (`reachable`, `equivalent`, `squash`, or `unproven`) and `preserved` flag. `matchingRef` and `matchingCommit` are optional witness details and are omitted from the aggregate when several targets are involved; `reason` is display-oriented explanatory text, not a stable machine decision key.
