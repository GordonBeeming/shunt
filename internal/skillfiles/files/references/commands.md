...
- `mounts` carries per-project host paths into the guest (typically user-secrets so the app's parameters resolve). Each entry is either a plain path string — `"~/.microsoft/usersecrets"` auto-maps the host home to the guest home (`/root/�…`) read-only, `"/abs"` maps to the same path — or the full `{ "host", "guest", "readOnly" }` object for read-write or a different guest path.
