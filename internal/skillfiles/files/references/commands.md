...
| `shunt-dev ls [-a] [--json]` | Sidings for the **current project**, with a `STATUS` column: `live` (front door points here), `up` (running + bridged, ready to `switch` to), `idle` (exists but not ready), or `stopped` (killed). `-a`/`--all` lists every project on the host. `--json` for tooling. |
