...
- `healthPort`/`healthPath` set the endpoint the **dashboard** hits (from inside the guest) to decide whether a siding is genuinely `running` vs merely `idle` (booted, not serving). Omit `healthPort` to default to the Aspire dashboard's home page (up whenever the AppHost is); for non-Aspire apps point it at a real health route, e.g. `"healthPort": 8080, "healthPath": "/healthz"`. `healthPath` defaults to `/`.
