...
| `shunt-dev restart [name\|host]` | Kill only the app process and rebuild it, keeping the guest + dockerd + dependency containers + data running. Use when hot reload misses a change. `restart host` (re)starts your **local** app natively — this is how you bring the host up after `switch host`, since `switch` only repoints and `restart`/`up` do the build + start. |
