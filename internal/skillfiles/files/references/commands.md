...
| `shunt-dev ls [-a] [--json]` | Sidings for the **current project**; `*` marks the live one. `-a`/`--all` lists every project on the host. `--json` for tooling. |
...
| `shunt-dev up <name>` | Build + start the app in the guest, then point the front door at it. Loads the project warm cache first (if any). First cold run is slow; see [iterating](iterating.md). `--no-bridge` starts it in the guest only (no host bridges, no front door) so you can verify it before going live; `--no-switch` bridges but doesn't repoint the front door. |
...
| `shunt-dev switch [name\|host]` | Point the stable front-door ports at a running siding (live, no restart), or at `host` to run the user's **local copy** natively from the main repo. No name — arrow-key picker (host listed first). Switching to `host` stops any siding target, removes this app's Caddy front-door servers so the native app binds the real ports (Caddy steps aside), and runs `aspire start` in the main repo; switching to a siding stops the host app first (best-effort) and re-binds the front door. **Always confirm with the user before switching** — it repoints the shared front door and can interrupt another agent/session. |
...
| `shunt-dev reapply [name]` | Recreate a siding's guest from the app's **saved** settings (`memory`/`cpus`/`mounts`/`env`), keeping its worktree, branch, and data clones. No name → the picker. Guest settings are baked in at creation, so an edit only reaches an existing siding via `reapply` then `up`. If you changed these in `.shunt.app.json` (rather than via `shunt config`), run `app update` first — `reapply` reads saved state; only `app add`/`app update` re-read the contract. (Front-door route changes use `app update`.) |
| `shunt-dev app add` | First-time registration of the repo with shunt (reads `.shunt.app.json`). Front-door ports are random+free by default (no collisions); `fixedPorts:true` pins them. For later contract edits use `app update`. |
| `shunt-dev app update` | Apply `.shunt.app.json` edits to the already-registered app, in place. Re-derives the front door and applies only the delta: added routes are registered and bridged onto the live siding, removed ones dropped, unchanged ones left untouched (no front-door blackout). Also refreshes saved guest settings (`memory`/`cpus`/`mounts`/`env`) from the contract — follow with `reapply` + `up` for those to take effect. Resolves the project case-insensitively, so it works from a differently-cased cwd. Errors (pointing at `app add`) if nothing's registered here yet. |
