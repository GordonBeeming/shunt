---
name: shunt
description: Run and edit an Aspire app inside an isolated shunt "siding" (a container instance with stable local ports) instead of the main repo. Use when starting a larger dev task in a shunt-managed Aspire repo, when the user wants to work in or switch between isolated instances, spin up a new track/siding, warm the dependency-image cache, or test changes without touching their main working copy. Triggers include "use shunt", "work in a siding", "spin up an instance", "switch instances", "isolated dev/track", or beginning substantial work in a repo that has a .shunt.app.json.
---

# shunt — isolated Aspire dev

shunt runs each experiment ("siding") of an Aspire app in its own isolated Apple `container` guest, with a stable set of local ports out front. You edit the siding's own git worktree on the host and the guest runs it live. The point: keep the user's main working copy and this conversation untouched while a bigger change runs in an isolated instance.
...
4. Run it via shunt, never a local `aspire start` (Aspire runs in the guest). The status script prints the right next command — usually `shunt-dev up <name>`; after edits, hot reload covers most changes and `shunt-dev restart <name>` does a full rebuild without dropping the env. See [iterating](references/iterating.md).
...
- Never run a local `aspire start`. Use `up` / `restart`.
