...
- Treat a reported active removal journal as authoritative. Source, lifecycle, data, sync/run/playwright, and trim mutations are fenced while removal resumes from its durable checkpoint; do not bypass the fence or manually delete partial resources. Shunt creates operation recovery refs before target deletion and removes them only after journal clear commits; later removal maintenance collects crash-orphaned recovery refs. Preserved targets also create deduplicated Shunt witness refs at accepted commits, while explicit-discard targets create no witness. Downgrading Shunt while a removal journal is active is unsupported.
