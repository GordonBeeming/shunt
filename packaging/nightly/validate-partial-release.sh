...
if ! jq -e --arg asset "$asset" --arg checksum "$checksum" '
  ([.assets[]?.name] | sort) as $names |
  ($names | unique) == $names and all($names[]; . == $asset or . == $checksum)
' "$release_json" >/dev/null; then
  printf 'draft release assets must be a unique subset of %s and %s\n' "$asset" "$checksum" >&2
  exit 1
fi

if jq -e --arg name "$asset" '[.assets[] | select(.name == $name)] | length == 1' "$release_json" >/dev/null; then
...
if jq -e --arg name "$checksum" '[.assets[] | select(.name == $name)] | length == 1' "$release_json" >/dev/null; then
